...
def downloadc25():
    ...
    for i in range(len(C25)):
        ...
        with open("{0}/stock{1}.csv".format(get_data_path(), C25s[i]), 'wb') as f:
            f.write(r.content)

    r = requests.get("http://euroinvestor.com{}".format(c20))
    with open("{0}/stock{1}.csv".format(get_data_path(),"c20"), 'wb') as f:
        f.write(r.content)
...
def loadc25():
    ...
    for company in C25s:
        tmp = pd.read_csv('{0}/stock{1}.csv'.format(get_data_path(), company))
        date = tmp["Date"]
        date_f = []
...
def loadc20():
    data = pd.read_csv('{0}/stock{1}.csv'.format(get_data_path(),"c20"))
    date = data["Date"]
    date_f = []
    for i in range(len(date)):
        date_f.append(pd.to_datetime(date[i][:10], format="%d/%m/%Y"))
